    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center md:p-6 overflow-hidden">
      ...
      <div className="relative z-50 w-full h-full md:h-auto md:max-w-[1000px] md:max-h-[90vh] bg-white dark:bg-background-dark shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-10 md:zoom-in duration-300">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-3">
                <h1 className="text-2xl md:text-4xl font-black tracking-tighter leading-none">{candidate.name}</h1>
              </div>
              <p className="text-[10px] md:text-xs font-black tracking-widest opacity-40">{candidate.title}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-20">
            <div className="lg:col-span-7 space-y-12">
                  ...
                  <ul className="space-y-6">
                    {insights.map((insight, idx) => (
                      <li key={idx} className="flex items-start gap-4">
                        ...
                        <span className="text-xs md:text-sm font-bold tracking-tight leading-relaxed">{insight}</span>
                      </li>
                    ))}
                  </ul>
                )}
              ...
              <section className="space-y-8">
                <h2 className="text-[10px] font-black tracking-[0.4em] opacity-40">Platform History</h2>
                <div className="space-y-8">
                  <div className="flex gap-6 border-l-2 border-black/10 dark:border-white/10 pl-6">
                    <div className="space-y-2">
                      <h3 className="text-xl md:text-2xl font-black tracking-tighter">{candidate.title}</h3>
                      <p className="text-[10px] font-black tracking-widest opacity-40">Primary Experience Pillar • {candidate.experience}</p>
                    </div>
                  </div>
                </div>
              </section>
            ...
            <div className="lg:col-span-5 space-y-12">
              <section className="space-y-8">
                <h2 className="text-[10px] font-black tracking-[0.4em] opacity-40">Skill Concentration</h2>
        ...
        <footer className="px-6 md:px-12 py-8 md:py-10 bg-black/5 dark:bg-white/5 border-t border-black/10 dark:border-white/10 flex flex-col md:flex-row items-center justify-between gap-8 shrink-0">
          <div className="flex items-center gap-4 text-[9px] font-black tracking-widest opacity-40">
            <span className="material-symbols-outlined text-base">visibility</span>
            Viewed by 4 network nodes
          </div>
          <div className="flex gap-4 w-full md:w-auto">
            <button onClick={onClose} className="flex-1 md:px-10 py-5 border border-black dark:border-white text-[10px] font-black tracking-widest hover:bg-red-500 hover:text-white hover:border-red-500 transition-all">
              Dismiss
            </button>
            <button className="flex-1 md:px-10 py-5 bg-black dark:bg-white text-white dark:text-black text-[10px] font-black tracking-widest hover:invert transition-all shadow-2xl flex items-center justify-center gap-3">
